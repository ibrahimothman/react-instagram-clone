...
import { togglePhotoLikes } from '../../services/Firebase'

function Actions({ docId, totalLikes, hasBeenLiked, handleFocus }) {

    const { user } = useUser()
    const [isLiked, setIsLiked] = useState(hasBeenLiked)
        ...
        <>
            ...
            <div className="p-4 py-0">
                    <p className="font-bold">
                        {likes.length === 1 ? `${likes} like` : `${likes} likes`}
                    </p>
            </div>
        </>
    )
